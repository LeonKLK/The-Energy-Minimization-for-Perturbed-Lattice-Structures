...
//  4_part3
//
//  Created by Leon Kwok on 6/12/2019.
...
int main() {
    ...
    //FILE *position;
    //position=fopen("position_fcc.xyz", "w");
    FILE *diffm;
    diffm=fopen("m_part3.csv", "w");
    nx=5;
    ny=5;
    nz=5;
    a=4.212;    //using angstroms as unit
    NoA=nx*ny*nz*4;
    ...
    double ***r_f = (double ***)malloc(2*NoA*sizeof(double**));
    for (i = 0; i< 2*NoA; i++) {
        r_f[i] = (double **) malloc(2*NoA*sizeof(double *));
        for (j = 0; j < 2*NoA; j++) {
    ...
    double ***g1 = (double ***)malloc(2*NoA*sizeof(double**));
    for (i = 0; i< 2*NoA; i++) {
        g1[i] = (double **) malloc(2*NoA*sizeof(double *));
        for (j = 0; j < 2*NoA; j++) {
            g1[i][j] = (double *)malloc(3*sizeof(double));
        }
    }
    
    double ***g2 = (double ***)malloc(2*NoA*sizeof(double**));
    for (i = 0; i< 2*NoA; i++) {
        g2[i] = (double **) malloc(2*NoA*sizeof(double *));
        for (j = 0; j < 2*NoA; j++) {
    ...
    while(m<100){
        ...
        for (k=0; k<2*NoA; k++) {
            for (j=0; j<2*NoA ; j++) {
             ...
             //    printf("%d %d %f %f %f %f %f %f %f\n", k,j,distance[k][j],-r[k][j][0],-r[k][j][1],-r[k][j][2],g1[k][j][0],g1[k][j][1],g1[k][j][2]);
            }
        }
        ...
        for (k=0;k<2*NoA;k+=2)
        {
            fprintf(position, "Mg\t %lf\t %lf\t %lf\n", p[k][0], p[k][1], p[k][2]);
            fprintf(position, "O\t %lf\t %lf\t %lf\n", p[k+1][0], p[k+1][1], p[k+1][2]);
        }*/
    }
    fclose(diffm);
